# SparkMinds Passport 智能打卡机系统

> 创智实验室 (SparkMinds) 硬件设备固件与综合管理工程，运行于 ARCS-MINI 硬件平台。

- 📖 **核心应用与详细文档**：[apps/sparkminds-passport/README.md](apps/sparkminds-passport/README.md)
- 📋 **学员发卡花名册**：[apps/sparkminds-passport/STUDENTS.md](apps/sparkminds-passport/STUDENTS.md)
- ⚡ **固件全量烧录规范**：[全量烧录规范.md](全量烧录规范.md)

---

## 🔄 免修改代码：串口 / ADB 一键切换学员身份 (Student Provision)

设备内置花名册解析引擎，**无需重新编译代码、无需重新烧录**，通过串口终端或 ADB 即可秒级切换任意学员账号（自动清空旧缓存并安全写入新密钥）：

### 1. 串口切换（推荐）
1. 串口终端连接设备（波特率 `115200`，8-N-1，如 `COM4`）；
2. 输入指令并回车：
   ```bash
   provision 002    # 一键切换为 陆昭闻（Owen）
   ```
   *(亦可输入完整学号 `provision SM-2026-002`，或其它学员编号如 `001`、`003`、`004`、`013` 等)*
3. 终端打印切换成功后，重启设备即可显示新学员身份。

### 2. ADB 切换
```powershell
# Windows
.\tools\adb\adb.exe shell provision 002

# Linux / Mac
./tools/adb/adb shell provision 002
```

---

## 平台基础概述

## 环境搭建

参考SDK的开发文档：
- [开发文档（在线）](https://docs2.listenai.com/arcs-sdk/v0.1.0/zh/html/get_started.html)

## 工程结构

### 顶层目录

```
voiceassistant/
├── apps/              # 应用方案工程集合
├── arcs-sdk/          # ARCS SDK 核心框架
├── modules/           # 项目级第三方模块
├── res/               # 资源文件（固件、配置、音频、图片等）
├── src/               # 语音交互核心代码
├── tools/             # 开发工具和脚本（如 adb、cskburn）
├── yaml/              # 配置文件
└── build.sh           # 工程构建脚本
```

### 详细目录说明

#### [apps/](apps/) - 应用程序

包含多个应用方案项目,详细说明请参考对应工程下的README.MD（如有）。

#### [src/](src/) - 语音交互核心代码

包含语音助手的核心业务逻辑、中间件和工具库：

```
src/
├── category/          # 品类相关代码
│   ├── comm/          # 品类通用代码
│   ├── evb/           # EVB 品类实现
│   └── yuba/          # YUBA 品类实现
├── framework/         # 核心框架
│   └── voice_msg.*    # 语音消息处理
├── middleware/        # 中间件层
├── server/            # 语音服务
├── shell/             # 调试命令行
│   └── cmd/           # Shell 命令实现
├── system/            # 系统服务
│   ├── platform.c     # 平台抽象层
│   └── remote_logger.*# 远程日志
└── utils/             # 工具库
```

**核心模块说明：**

- **category**: 不同产品品类的特定实现，封装品类差异
  - **comm**: 品类间通用代码和共享组件
- **framework**: 核心框架，提供语音消息传递机制
- **middleware**: 提供硬件抽象和系统服务，供应用层调用
- **server**: 语音交互核心逻辑，包括云端通信和唤醒处理
- **shell**: 提供调试命令行接口，用于开发调试
- **system**: 系统级服务，包括平台抽象和远程日志
- **utils**: 通用工具库，提供事件、配置、编解码等基础功能


## 快速开始

### 编译工程

基本编译命令,这里以/apps/evb-arcs项目为例,用户根据自己的实际项目路径选择：

```bash
./build.sh -S ./apps/arcs-evb -C
```

编译成功后，固件文件位于 [build/arcs-evb.bin](build/arcs-evb.bin)

### 固件烧录

#### FLASH 分区说明
参考对应app的资源分配置表说明文件

#### 烧录方式

##### 方式一: 串口烧录

适用于首次烧录或 boot 程序损坏的情况。

**进入 ROM 升级模式：**

1. 将开发板的 `boot` 引脚接地
2. 复位开发板
3. 开发板进入 ROM 升级模式

**烧录 boot 程序：**

```bash
# 连接串口 UART0(GPIOA2_RX0/GPIOA3_TX0)
cskburn -C arcs -s /dev/ttyACM0 -b 3000000 0x0 ./res/res_pack_a/boot.bin
```

> **注意：** boot 引脚和烧录串口引脚复用，烧录前需将 boot 引脚悬空

##### 方式二: ADB 烧录（推荐）

适用于已有 boot 程序且 ADB 可用的设备。仓库已内置 ADB 工具：

- Windows: `tools/adb/adb.exe`
- Linux / WSL: `tools/adb/adb`

烧录脚本会自动检测 ADB、让普通模式设备进入 recovery、等待 `BOOT-*` 设备出现、推送固件并重启设备。多设备或多个相同 `BOOT-*` 序列号时，脚本会优先用 `transport_id` 区分目标设备。开发者通常不需要手动执行 `adb shell recovery`。

**Linux / WSL / Git Bash：**

```bash
# 默认模式：烧录除 boot 之外的其他固件和资源
bash adb_download.sh -S res/arcs-mini

# app 模式：只烧录 CP app 固件
bash adb_download.sh -S res/arcs-mini app

# boot 模式：包含 boot 的完整烧录
bash adb_download.sh -S res/arcs-mini boot
```

**Windows PowerShell：**

```powershell
# 默认模式：烧录除 boot 之外的其他固件和资源
powershell -ExecutionPolicy Bypass -File adb_download.ps1 -S res/arcs-mini

# app 模式：只烧录 CP app 固件
powershell -ExecutionPolicy Bypass -File adb_download.ps1 -S res/arcs-mini app

# boot 模式：包含 boot 的完整烧录
powershell -ExecutionPolicy Bypass -File adb_download.ps1 -S res/arcs-mini boot
```

**三种烧录模式：**

| 模式 | 命令参数 | 烧录内容 | 适用场景 |
| --- | --- | --- | --- |
| 默认 | 无 | `ap.bin`、`tone.bin`、`wake_word.bin`、`emoji.bin`、`respak.bin`、`build/arcs-mini.bin` | 静态资源或 AP/app 一起更新 |
| app | `app` | `build/arcs-mini.bin` | 只修改 CP app / 应用代码 |
| boot | `boot` | `boot.bin` + 默认模式全部内容 | 首次烧录、boot 变更或 boot 损坏恢复 |

`-S` 指向包含 `partition_table.json` 的资源目录；脚本会按该分区表读取镜像地址和文件路径，跳过没有 `file` 的保留分区。`-B` 或 `BUILD_DIR` 可覆盖 `${BUILD_DIR}` 对应的构建输出目录。

`arcs-mini` 的分区映射来自 [res/arcs-mini/partition_table.json](res/arcs-mini/partition_table.json)：

| 分区 | 地址 | 文件 |
| --- | --- | --- |
| boot | `0x000000` | `res/arcs-mini/boot.bin` |
| ap | `0x040000` | `res/arcs-mini/ap.bin` |
| tone | `0x100000` | `res/arcs-mini/tone.bin` |
| wake_word | `0x200000` | `res/arcs-mini/wake_word.bin` |
| emoji | `0x380000` | `res/arcs-mini/emoji.bin` |
| respak | `0x440000` | `res/arcs-mini/respak.bin` |
| app (CP) | `0x600000` | `build/arcs-mini.bin` |

**可选环境变量：**

| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `MAX_DEVICES` | `10` | 最多支持的设备数量 |
| `RECOVERY_TIMEOUT` | `120` | 等待进入 recovery 的超时秒数 |
| `POLL_INTERVAL` | `2` | 轮询 recovery 状态的间隔秒数 |
| `ADB_CMD` | 自动检测 | 手动指定 ADB 路径 |
| `BUILD_DIR` | `build` | `arcs-mini.bin` 所在构建目录 |
| `RES_DIR` | `res/arcs-mini` | 默认资源目录，可被 `-S` 覆盖 |

**ADB 工具选择顺序：**

1. `ADB_CMD` 环境变量指定的路径
2. 仓库内置 `tools/adb/adb` 或 `tools/adb/adb.exe`
3. 系统 PATH 中的 `adb`

**烧录建议：**

- 只改应用代码时，优先使用 `app` 模式，避免重复写入静态资源。
- 修改提示音、唤醒词、表情、respak 或 AP 固件时，使用默认模式。
- 只有首次烧录、boot 变更或 boot 损坏恢复时，才使用 `boot` 模式。

## 常见问题

### 编译问题

**Q: 找不到工具链**
```
A: 检查环境变量是否正确设置：
   echo $NUCLEI_TOOLCHAIN_PATH
   echo $LISTENAI_TOOLS_PATH
```

**Q: 编译失败**
```
A: 尝试清理后重新编译：
   ./build.sh -C
```

### 烧录问题

**Q: ADB 连接不上设备**
```
A: 1. 检查 USB 连接
   2. 运行 adb devices 查看设备
   3. 尝试重启 adb: adb kill-server && adb start-server
```

**Q: 烧录后无法启动**
```
A: 1. 检查是否烧录了所有必要分区
   2. 查看串口日志定位问题
   3. 尝试重新烧录 boot.bin
```

## 版本信息

- **当前分支**: master
- **主分支**: master

## 参考文档

- [ARCS SDK 文档](arcs-sdk/README.md)
- [ebus 模块文档](modules/ebus/readme.md)
- [lschat 模块文档](modules/lschat/README.MD)

## 许可证

本项目遵循相应的开源许可证，详见各模块的 LICENSE 文件。
